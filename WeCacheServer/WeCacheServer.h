...
#ifndef _WECACHESERVER_H_
#define _WECACHESERVER_H_
...
class WeCacheServer:public TCPServer
{
public:
    WeCacheServer(ConfReader *conf):TCPServer(conf){}
protected:
    // @override
...
private:
    ...
    // handler指针类型定义
    typedef int (WeCacheServer::*HANDLER_PTR)(TCPSession *session, const char *data, uint32_t head_size, uint32_t body_size, uint64_t tid);
     ...
     **********  注册 handler  **********
    /***********************************/
    HANDLE_REG(WeCacheServer, uint32_t, HANDLER_PTR)
#if USE_EXAMPLE == 1
